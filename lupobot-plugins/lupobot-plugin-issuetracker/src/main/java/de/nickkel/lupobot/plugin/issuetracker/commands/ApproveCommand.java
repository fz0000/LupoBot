...
@CommandInfo(name = "approve", category = "judger")
...
public class ApproveCommand extends LupoCommand {
    ...
    public void onCommand(CommandContext context) {
        ...
        if (issue != null) {
            if (issue.hasApproved(context.getMember())) {
                sendSyntaxError(context, "issuetracker_approve-already-done", issue.getId());
                return;
            } else if (issue.hasApproved(context.getMember())) {
                sendSyntaxError(context, "issuetracker_approve-already-declined", issue.getId());
                return;
            }
            ...
            issue.approve(context.getMember(), message, priority, false);
        } else {
